...
export async function createArticle(articleData: Omit<Article, 'id' | 'slug' | 'createdAt' | 'updatedAt' | 'readingTime'>): Promise<Article> {
...
export async function updateArticle(slug: string, articleData: Partial<Article>): Promise<Article | null> {
  ...
  const updatedArticle: Article = {
    ...
    readingTime: articleData.content 
      ? calculateReadingTime(articleData.content) 
      : article.readingTime
  };
...
export async function addComment(articleSlug: string, author: { name: string; email: string }, content: string): Promise<Comment | null> {
